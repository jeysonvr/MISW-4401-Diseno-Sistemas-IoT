...
class RemaView(TemplateView):
    ...
    def get(self, request, **kwargs):
        ...
        return get_map_json_reto(request, **kwargs)
...
def get_map_json_reto_bloqueaEC2(request, **kwargs):
    data_result = {}
    ...
    location = None
    if locations != None:
        location = Location.objects.filter(country=1)[0]
    ...
    filteredData= Data.objects.filter(
            station__in=station, measurement__name=selectedMeasure.name,  time__gte=start.date(), time__lte=end.date())
    
    for valueData in filteredData:
        if valueData.value > 10 and valueData.value < 10.05 :
            data.append({
        ...
        data_result["measurement"] = selectedMeasure.name
...
def get_map_json_reto(request, **kwargs):
    ...
    if measureParam != None:
        selectedMeasure = Measurement.objects.filter(name=measureParam)[0]
    data_result["measurements"] = [loc.str() for loc in measurements]
    data_result["measurement"] = selectedMeasure.name
    ...
    location = None
    if locations != None:
        location = Location.objects.filter(country_id=int(request.GET.get("country", None)))[0]
    elif locations.count() > 0:
        location = locations[0]

    data_result["locations"] = [loc.str() for loc in locations]
    data_result["location"] = location.country.name
    ...
    #data_result["station"] = station.user

    filteredData= Data.objects.filter(
            station__in=station, measurement__name=selectedMeasure.name,  time__gte=start.date(), time__lte=end.date())
    
    data_result["dataCount"] = filteredData.count()
