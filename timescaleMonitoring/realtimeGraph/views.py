...
def get_station_json(request, **kwargs):
    data_result = {}

    stationParam = kwargs.get("station", None)
    selectedStation = None
    stationLocation = None
    stations = Station.objects.all()
    measurements = Measurement.objects.all()

    if stationParam != None:
        selectedStation = Station.objects.filter(id=stationParam)[0]
    ...
    for measurement in measurements:
        measurementData = Data.objects.filter(
            station__id=selectedStation.id, measurement__name=measurement.name,  time__gte=start_ts, time__lte=end_ts)

        if measurementData.count() <= 0:
            continue
        amountVal = measurementData.aggregate(Sum("length"))["length__sum"]
        minVal = measurementData.aggregate(Min("min_value"))["min_value__min"]
        maxVal = measurementData.aggregate(Max("max_value"))["max_value__max"]
        avgVal = measurementData.aggregate(Avg("avg_value"))["avg_value__avg"]
        ...
        data.append({
            ...
            'min': minVal if minVal != None else 0,
            'max': maxVal if maxVal != None else 0,
            'avr': round(avgVal if avgVal != None else 0, 2),
        })

    stationLocation = Location.objects.filter(id=selectedStation.location_id)[0]
    startFormatted = start.strftime("%d/%m/%Y") if start != None else " "
    endFormatted = end.strftime("%d/%m/%Y") if end != None else " "

    data_result["start"] = startFormatted
    data_result["end"] = endFormatted
    data_result["location"] = f'{stationLocation.city.name}, {stationLocation.state.name}, {stationLocation.country.name}'
    data_result["lat"] = stationLocation.lat
    data_result["lng"] = stationLocation.lng
    data_result["data"] = data
