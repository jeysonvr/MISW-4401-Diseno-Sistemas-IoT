...
def get_station_json(request, **kwargs):
    ...
    for measurement in measurements:
        measurementData = Data.objects.filter(
            station__id=selectedStation.id, measurement__name=measurement.name,  time__gte=start.date(), time__lte=end.date())

        if measurementData.count() <= 0:
            continue
        minVal = measurementData.aggregate(
            Min('value'))['value__min']
        maxVal = measurementData.aggregate(
            Max('value'))['value__max']
        avgVal = measurementData.aggregate(
            Avg('value'))['value__avg']

        data.append({
            'measurement': measurement.name,
            'unit': measurement.unit,
            'amount': measurementData.count(),
            'min': minVal if minVal != None else 0,
            'max': maxVal if maxVal != None else 0,
